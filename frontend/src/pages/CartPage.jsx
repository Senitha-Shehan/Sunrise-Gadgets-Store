...
function CartPage() {
      ...
      <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface-50)', padding: '24px' }}>
        <div style={{ textAlign: 'center', maxWidth: '380px' }}>
          ...
          <p style={{ color: '#64748b', marginBottom: '24px', fontSize: '0.95rem' }}>
            Browse our products and add items you are interested in.
          </p>
  ...
  return (
    <div style={{ background: 'var(--surface-50)', minHeight: '100vh', paddingBottom: isMobile ? '120px' : '60px' }}>
      {/* Header */}
      <div style={{ background: 'white', borderBottom: '1px solid var(--surface-100)', position: 'sticky', top: 0, zIndex: 10, boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 16px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', color: 'var(--surface-900)' }}>
              🛒 My Cart ({cartItems.length})
            </span>
            <button onClick={clearCart}
              style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444', fontWeight: 600, fontSize: '0.78rem', fontFamily: 'var(--font-sans)', minHeight: 'auto' }}
            >
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(1.2rem,4vw,1.6rem)', color: 'var(--surface-900)', letterSpacing: '-0.03em', marginBottom: '4px' }}>
              Your Order
            </h1>
            <p style={{ color: '#64748b', marginBottom: '8px', fontSize: '0.875rem' }}>
            ...
            {cartItems.map(item => (
              <div key={item._id} style={{
                ...
                border: '1.5px solid var(--surface-100)', padding: '14px',
                display: 'flex', gap: '12px', alignItems: 'flex-start',
                boxShadow: '0 2px 12px rgba(0,0,0,0.04)',
              }}>
                {/* Tappable image → product detail */}
                <button
                  onClick={() => navigate(`/product/${item._id}`)}
                  style={{
                    width: '72px', height: '72px', borderRadius: '10px', overflow: 'hidden',
                    background: 'var(--surface-50)', flexShrink: 0, border: '1px solid var(--surface-100)',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '0.68rem', color: 'var(--brand-600)', fontWeight: 600, marginBottom: '2px' }}>{item.brand}</div>
                  <div style={{
                    fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--surface-900)',
                    fontSize: '0.9rem', lineHeight: 1.3, marginBottom: '10px',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid var(--surface-200)', borderRadius: '10px', overflow: 'hidden' }}>
                      <button
                        ...
                        style={{
                          ...
                          fontFamily: 'var(--font-sans)', minHeight: 'unset',
                          touchAction: 'manipulation',
                        }}
                      >−</button>
                      <span style={{
                        minWidth: '36px', textAlign: 'center', fontSize: '0.9rem',
                        fontWeight: 700, color: 'var(--surface-900)',
                        borderLeft: '1px solid var(--surface-200)', borderRight: '1px solid var(--surface-200)',
                        lineHeight: '38px', padding: '0 4px',
                      }}>{item.qty}</span>
                      <button
                        onClick={() => updateQty(item._id, item.qty + 1)}
                        style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    minHeight: 'unset', transition: 'all 0.15s', touchAction: 'manipulation',
                  }}
                  title="Remove"
                >
          ...
          {/* Order Summary — desktop sidebar / hidden on mobile (shown in sticky bar) */}
          {!isMobile && (
            <div style={{
              background: 'white', borderRadius: '20px', border: '1.5px solid var(--surface-100)',
              ...
              <p style={{ fontSize: '0.75rem', color: '#94a3b8', marginBottom: '16px', lineHeight: 1.6, background: 'var(--surface-50)', padding: '10px 12px', borderRadius: '10px' }}>
                💡 This is a quote enquiry. Our team will confirm pricing and availability via WhatsApp or email.
              </p>
              ...
              <button 
                ...
                style={{
                  ...
                  width: '100%', padding: '16px', background: 'var(--brand-600)', color: 'white',
                  borderRadius: '12px', border: 'none', fontWeight: 800,
                  fontSize: '1rem', fontFamily: 'var(--font-display)', marginBottom: '12px',
                  boxShadow: '0 8px 24px rgba(6,182,212,0.3)', transition: 'all 0.2s', cursor: 'pointer',
                }}
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--brand-700)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--brand-600)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                🚀 Proceed to Checkout
              </button>

              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '16px 0' }}>
                <div style={{ flex: 1, height: '1px', background: 'var(--surface-100)' }} />
                <span style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--surface-300)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>OR ENQUIRE VIA</span>
                <div style={{ flex: 1, height: '1px', background: 'var(--surface-100)' }} />
              </div>
              ...
              <a href={buildWhatsAppMessage()} target="_blank" rel="noopener noreferrer"
                style={{
                  ...
                  fontSize: '0.95rem', fontFamily: 'var(--font-display)', marginBottom: '10px',
                  boxShadow: '0 4px 16px rgba(37,211,102,0.35)', transition: 'all 0.2s', boxSizing: 'border-box',
                }}
              ...
              <a href={`mailto:info@sunrisegadgets.lk?subject=Product%20Enquiry&body=${encodeURIComponent(buildEmailBody())}`}
                style={{
                  ...
                  width: '100%', padding: '12px', background: 'var(--surface-50)', color: '#475569',
                  borderRadius: '12px', textDecoration: 'none', fontWeight: 600,
                  fontSize: '0.875rem', fontFamily: 'var(--font-sans)',
                  border: '1.5px solid var(--surface-200)', transition: 'all 0.2s', boxSizing: 'border-box',
                }}
              >
      ...
      {isMobile && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <div>
              <div style={{ fontSize: '0.72rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Estimated Total</div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.3rem', color: 'var(--brand-600)', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              ...
              style={{
                ...
                padding: '14px', background: 'var(--brand-600)', color: 'white',
            ...
            <a
              ...
              style={{
                ...
                background: '#25D366', borderRadius: '14px', textDecoration: 'none', color: 'white',
                flexShrink: 0, touchAction: 'manipulation', boxShadow: '0 4px 12px rgba(37,211,102,0.2)'
              }}
            ...
            <a
              href={`mailto:info@sunrisegadgets.lk?subject=Product%20Enquiry&body=${encodeURIComponent(buildEmailBody())}`}
              style={{
                width: '52px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: 'var(--surface-50)', border: '1.5px solid var(--surface-200)',
                borderRadius: '14px', textDecoration: 'none', color: '#475569',
